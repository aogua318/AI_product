// 函数 0x493abb  __close_nolock  size=0x9C  模块=CRT与库
...
int __cdecl _close_nolock(int FileHandle)
{
  ...
  if ( _get_osfhandle(FileHandle) == -1
    || (FileHandle == 1 && (*(_BYTE *)(dword_8F61C0[0] + 132) & 1) != 0
     || FileHandle == 2 && (*(_BYTE *)(dword_8F61C0[0] + 68) & 1) != 0)
  ...
  *(_BYTE *)(dword_8F61C0[FileHandle >> 5] + ((FileHandle & 0x1F) << 6) + 4) = 0;
  if ( !LastError )
    return 0;
