// 函数 0x48dea5  ?FindHandler@@YAXPAUEHExceptionRecord@@PAUEHRegistrationNode@@PAU_CONTEXT@@PAXPBU_s_FuncInfo@@EH1@Z  size=0x375  模块=CRT与库
          ...
          if ( *v15 > 0 )
          {
            do
            {
                     ...
                     (int)&std::bad_exception `RTTI Type Descriptor') )
              {
                ...
                std::exception::exception((std::exception *)pExceptionObject, (const char *const *)&ExceptionRecord);
...
$NextTryBlock$28284:
  ...
  {
    pESTypeList = v20->pESTypeList;
    if ( pESTypeList )
    {
      if ( !IsInExceptionSpec(pESTypeList, v11) )
      {
        _getptd();
        _getptd();
